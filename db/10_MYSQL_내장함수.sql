/*
if(수식, 참, 거짓)
ifnull(속성, 값) : 속성 값이 null이 아니면 속성값을 이용하고, null이면 값을 이용함.
case when 조건1 them 값1 when 조건2 then 값2 else 값3
...
right(문자열, 숫자);
: 문자열에서 오른쪽에서 숫자개수만큼 문자열을 추출 오른쪽/왼쪽 기준 고정길이의 문자열 추출시 유용

lpad/rpad(문자열, 숫자, 채울문자열);
: 문자열을 채울문자열을 왼쪽/오른쪽에 채워서 숫자개수만큼 길이가 되도록 함.
