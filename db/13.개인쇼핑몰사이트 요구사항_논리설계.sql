...
- 회원은 리뷰 등록시, 별점(1~5)과 리뷰 내용, 이미지(최대 1개)를 등록해서 리뷰를 남긴다.
...
-- -- 관계에 속성이 없는 경우 --
두 개체의 관계가 1:1인 경우 
	=> A개체에 외래키로 B개체의 기본키를 설정. 누가 A,B가 될 지 상관없음
두 개체의 관계가 1:n인 경우 
	=> n쪽인 개체에 다른 개체의 기본키를 외래키를 설정.
두 개체의 관계가 n:m인 경우
	=> 새 릴레이션을 추가 후, 대리키를 기본키로 만들고, 각 개체의 기본키들을 새 릴레이션의 외래키로 설정.
    
    -- 관계에 속성이 있는 경우 --
두 개체의 관계가 1:1, 1:n, n:m 상관없음
	=> 새 릴레이션을 추가 후, 관계에 있는 속성을 새 릴레이션 추가하고, 두 개체의 기본키들을
    새 릴레이션의 외래키로 지정
*/
/*
