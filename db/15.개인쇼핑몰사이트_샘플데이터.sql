...
-- 서울시 강남구 KH빌딩, 주소 상세가 6층 361반, 우편번호가 01234인 회원이 가입을 했을 때 쿼리 작성
-- 권한은 ADMIN, USER 중 하나
...
-- 제품코드가 ABC001인 제품 중에서 유선 제품이 10개 들어오고, 무선 제품이 5개 들어왔을 때 사용해야하는 쿼리
...
AND BA_OP_NUM = (SELECT OP_NUM FROM `OPTION` WHERE OP_PR_CODE = 'ABC123' AND OP_NAME = '유선');
...
-- 3. 제품 수량을 수정
UPDATE `OPTION` SET OP_AMOUNT = OP_AMOUNT - 2 WHERE OP_NUM = 1;
UPDATE `OPTION` SET OP_AMOUNT = OP_AMOUNT - 3 WHERE OP_NUM = 3;
-- 4. 마일리지(포인트) 적립
-- 4-1. 마일리지 내역 추가
INSERT INTO POINT(PO_CONTENT, PO_AMOUNT, PO_ME_ID)
