...
BEGIN
    ...
    SET _ORDER_CHAR = (SELECT LPAD(
    (SELECT COUNT(*) + 1 FROM STUDENT WHERE NUM LIKE CONCAT(_YEAR, _DEP_CODE, "%")  ) , 3, '0'));
    
    SET _STUDENT_NUM = CONCAT(_YEAR, _DEP_CODE, _ORDER_CHAR); -- 문자열을 이어붙이는 함수 CONCAT
    -- SELECT _STUDENT_NUM; 
END //
DELIMITER ;
