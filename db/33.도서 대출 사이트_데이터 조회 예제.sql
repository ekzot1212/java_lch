...
AND LO_RETURN_DATE IS NULL;

-- 각 회원들의 대출이 불가능한 기간을 조회하는 쿼리
-- 대출이 가능한 회원은 대출 가능으로 표시하고,
-- 대출이 불가능한 회원은 언제까지 인지를 표시 하도록 작성
SELECT ME_ID, 
	CASE 
...
SELECT BO_TITLE ,COUNT(LO_NUM) AS 대여횟수
FROM BOOK
LEFT JOIN LOAN ON BO_NUM = LO_BO_NUM
GROUP BY BO_NUM;

-- 회원별 연체 회수를 조회하는 쿼리
SELECT ME_ID, COUNT(LO_EXPECTED_DATE < LO_RETURN_DATE) AS 연체회수
FROM MEMBER
LEFT JOIN LOAN ON LO_ME_ID = ME_ID
GROUP BY ME_ID;

-- 도서를 가장 많이 대여한 회원을 조회하는 쿼리, 대출수가 같은 경우 아이디순으로 정렬 후 첫번째 회원을 선택
SELECT ME_ID AS 아이디, COUNT(LO_NUM) AS 대여횟수
FROM MEMBER
JOIN LOAN ON ME_ID = LO_ME_ID
GROUP BY ME_ID
ORDER BY 대여횟수 DESC, 아이디 ASC
LIMIT 1;
