...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalFileName, byte[] fileData) throws IOException {
		...
		UUID uuid = UUID.randomUUID(); // 범용 고유식별자 (8-4-4-4-12_파일명)
		String savedFileName = uuid.toString() + "_" + originalFileName;
		
		// 한 폴더에 파일이 몰리지 않게 날짜별로 업로드 파일을 관리
		//2023/09/01 와 같이 폴더가 만들어지게
	...
	private static String calcPath(String uploadPath) {
		...
		// \\2023
		String yearPath = File.separator + cal.get(Calendar.YEAR); //File.separator = 운영체제의 구분자
		// \\2023\\09
		String monthPath = yearPath + File.separator + new DecimalFormat("00").format(cal.get(Calendar.MONTH)+1); //월을 8이아닌 08로 출력하기 위해
		// \\2023\\09\\01
